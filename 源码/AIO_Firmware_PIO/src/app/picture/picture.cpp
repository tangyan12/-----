...
/*
	****注释****
write_config、read_config都是从flash内读写一个uint32类型数据
该数据控制自动切换图片的时间间隔，0不切换，默认10000毫秒，存储是将uint32
转换为字符串，读取将字符串转为整形
...
{
	//创建一个屏幕将其变为透明但没有加载
    photo_gui_init();//创建了一个对象，添加了样式
    // 获取配置信息，只获取一个刷新间隔时间参数
